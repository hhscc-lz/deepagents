...
对复杂目标使用此工具，确保跟踪关键阶段并让用户了解整体进度。
任务应是宏观的阶段（如"数据查询"、"分析撰写"），而不是每一步操作的细化。
...
- 任务数量控制在 2-5 个，代表工作的主要阶段，而非具体操作步骤
...
4. **任务粒度**：
   - 任务代表工作的主要阶段，例如"数据查询"、"分析撰写"、"导出报告"
   - 不要将单次工具调用或单条查询拆成独立任务
   - 目标是 2-5 个任务，让用户看到整体进度，而非执行细节
